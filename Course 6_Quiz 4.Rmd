...
n<-9
stdev<-30
#Interval = mean+- (tn-1)*(s/sqrt(n))
mean+c(-1,1)*qt(.975,df=n-1)*(stdev/sqrt(n))
...
```{r}
#assume coke lovers is 1 and pepsi lover is 0
binom.test(3,n=4,p=0.5,alternative="greater")
```
Answer: p-value=0.3125
...
```{r}
# The 1 is for the pill and the 2 is the placebo
n1<-9
n2<-9
mean1<- -3
mean2<- 1
...
s<-((n1-1)*stdev1^2+(n2-1)*stdev2^2)/(n1+n2-2)
pt((mean1-mean2)/(s*sqrt(1/n1+1/n2)),df=n1+n2-2)
```
Answer: 0.0035
...
power.t.test(n,delta=mean,sd=stdev,alternative ="one.sided",sig=0.05,
             type="one.sample")
```
Answer: power=0.799
...
power.t.test(delta=mean,sd=stdev,power=power,sig.level = 0.05,
             alternative = "one.sided",type="one.sample")$n
```
Answer: 138.4
